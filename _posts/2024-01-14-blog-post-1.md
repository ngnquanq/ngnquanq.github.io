...
Trong đó thì $\oplus$ là phép toán concatenation (phép toán ghép) và để khái quát hơn cho trường hợp của một câu, lấy ví dụ ta không quan tâm tới toàn bộ câu, mà chỉ một đoạn nhỏ trong câu thôi thì ta cũng có thể biểu diễn dưới công thức toán như sau: $\mathbf{x}_{(i:j)} = \mathbf{x}_i \oplus \mathbf{x}_{i+1} \oplus \mathbf{x}_{i+2} \oplus \ldots \oplus \mathbf{x}_j$. Như hình minh họa phía dưới:
...
Giờ mọi người đã biết đầu vào là gì, filter trong mô hình này là gì, giờ mình đi tới đầu ra 
...
* Ưu điểm:
    * Chi phí tính toán thấp: Các phép tích chập trong các mạng CNN được ưa chuộng một phần bởi vì chúng có ít tham số, tham số của các lớp tích chập này cũng chính là số lượng phần tử trong cái filter đó, lấy ví dụ các bạn có một filter 3x3 thì số lượng phần tử trong cái filter đó là 9.
